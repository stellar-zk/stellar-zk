...
1. Resolve customization: read `{skill-root}/customize.toml`, then merge team overrides from `{project-root}/.stellar-build/custom/product-brief.toml` and personal overrides from `{project-root}/.stellar-build/custom/product-brief.user.toml`, if they exist (scalars override, tables deep-merge, arrays of tables keyed by `code`/`id` replace matching entries and append new ones, other arrays append).
...
5. Load config with a layered per-key lookup (each key comes from the first source that defines it): `{project-root}/.stellar-build/bmm/config.yaml` (and `config.user.yaml`), then `~/.stellar-build/config.yaml` (global, installer-written; in sandboxed installs it lives under the install prefix). Resolve `{user_name}`, `{communication_language}`, `{document_output_language}`, `{planning_artifacts}`, `{project_name}`, `{date}`. Missing keys → neutral defaults (`{planning_artifacts}` → `{project-root}/docs`); never block.
