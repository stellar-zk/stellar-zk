...
Resolve config with a layered per-key lookup (each key comes from the first source that defines it): `{project-root}/.stellar-build/bmm/config.yaml` (+ `config.user.yaml`), then `~/.stellar-build/config.yaml` (global, installer-written; in sandboxed installs it lives under the install prefix), then built-in defaults (`{user_name}` from `git config user.name`, languages English, `{user_skill_level}` intermediate, `{implementation_artifacts}` `{project-root}/docs/stories`). Never block on missing config files. Resolve:
...
<workflow>
  ...
  <action>Resolve `workflow.on_complete` from the customization files (base `{skill-root}/customize.toml`, then team/personal overrides under `{project-root}/.stellar-build/custom/`) — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
  </step>
